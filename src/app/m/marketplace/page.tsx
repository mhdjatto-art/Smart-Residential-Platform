...
const KIND_GROUPS: { titleKey: "mobile.marketplace_home_services" | "mobile.marketplace_food_groceries" | "mobile.marketplace_wellbeing" | "mobile.marketplace_other"; kinds: string[] }[] = [
  { titleKey: "mobile.marketplace_home_services", kinds: ["maintenance","plumbing","electrician","ac_technician","cleaning"] },
  { titleKey: "mobile.marketplace_food_groceries", kinds: ["grocery","restaurant","delivery"] },
  { titleKey: "mobile.marketplace_wellbeing", kinds: ["pharmacy","laundry","car_wash","moving"] },
  { titleKey: "mobile.marketplace_other", kinds: ["security","internet_services","other"] },
];
      ...
      <div className="p-4 space-y-5">
        ...
        {providers.length === 0 ? (
          <div className="py-8 text-center text-sm text-muted-foreground">
            ...
            <p className="mt-2">{t("mobile.marketplace_no_providers")}</p>
          </div>
        ) : (
          KIND_GROUPS.map((group) => {
            ...
            return (
              <section key={group.titleKey}>
                <h2 className="mb-2 text-sm font-semibold text-muted-foreground">{t(group.titleKey)}</h2>
                  ...
                  {list.map((p) => (
                    <Link key={p.id} href={`/m/marketplace/providers/${p.id}`} className="rounded-xl border bg-card p-3">
                      ...
                      {p.rating_count > 0 ? (
                        ...
                        <p className="mt-1 text-[11px] text-muted-foreground">{t("mobile.no_reviews")}</p>
                      )}
                    </Link>
                  ))}
                </div>

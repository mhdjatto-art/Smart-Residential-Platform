...
export default async function MobileComplaintDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <MobileTopbar title={ticket.ticket_number} userId={ctx.user_id} unread={0} showBack />
      <div className="p-4 space-y-4">
        <div className="rounded-xl border bg-card p-4">
          ...
          <div className="mt-3"><LiveTicketStatus ticketId={ticket.id} initialStatus={ticket.status} /></div>
          ...
          {ticket.resolution_notes && (
            <div className="mt-4 rounded-md bg-emerald-50 dark:bg-emerald-950/40 p-3 text-sm">
              <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">{t("mobile.resolution")}</p>

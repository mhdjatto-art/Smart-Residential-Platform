      ...
      <div className="rounded-lg border bg-white p-8 text-slate-900 shadow-sm print:rounded-none print:border-0 print:shadow-none dark:bg-white">
        ...
        <header className="border-b border-slate-200 pb-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-widest text-slate-500">{t("mobile.receipt_title")}</p>
            ...
            <div className="text-right">
              <div className="inline-flex items-center gap-2 rounded-full bg-emerald-100 px-3 py-1 text-sm font-semibold text-emerald-800">
                <CheckCircle2 className="h-4 w-4" /> {r.bill.status === "paid" ? t("mobile.receipt_paid_status") : r.bill.status.toUpperCase()}
              </div>
              <p className="mt-2 text-xs text-slate-500">{t("mobile.receipt_number")}</p>
        ...
        <section className="grid grid-cols-1 gap-6 py-6 sm:grid-cols-2">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">{t("mobile.receipt_billed_to")}</p>
            <p className="mt-1 text-base font-semibold">{r.resident?.full_name ?? t("mobile.receipt_resident")}</p>
            {r.unit && (
              <p className="text-sm text-slate-600">
          ...
          <div className="text-right">
            <p className="text-xs uppercase tracking-wide text-slate-500">{t("mobile.receipt_service")}</p>
            ...
            <p className="text-xs text-slate-500">
              {t("mobile.receipt_period", { start: formatDate(r.bill.billing_period_start), end: formatDate(r.bill.billing_period_end) })}
            </p>
            <p className="text-xs text-slate-500">{t("mobile.receipt_due_date", { date: formatDate(r.bill.due_date) })}</p>
          </div>
        </section>
        ...
        <section className="border-t border-slate-200 py-6">
          <table className="w-full text-sm">
            <tbody>
              <tr>
                <td className="py-1 text-slate-600">{t("mobile.receipt_subtotal")}</td>
                <td className="py-1 text-right tabular-nums">{formatCurrency(r.bill.subtotal, { currency: r.bill.currency })}</td>
              </tr>
              <tr>
                <td className="py-1 text-slate-600">{t("mobile.receipt_tax")}</td>
              ...
              {r.bill.penalty_amount > 0 && (
                <tr>
                  <td className="py-1 text-amber-700">{t("mobile.receipt_late_penalty")}</td>
              ...
              <tr className="border-t border-slate-200">
                <td className="py-2 font-semibold">{t("mobile.receipt_total")}</td>
              ...
              <tr className="bg-emerald-50">
                <td className="py-2 font-semibold text-emerald-800">{t("mobile.receipt_paid")}</td>
        ...
        <section className="border-t border-slate-200 pt-6">
          <p className="text-xs uppercase tracking-wide text-slate-500">{t("mobile.receipt_payment_details")}</p>
          <dl className="mt-2 grid grid-cols-2 gap-2 text-sm">
            <dt className="text-slate-600">{t("mobile.receipt_paid_at")}</dt>
            <dd className="text-right tabular-nums">{paidPretty}</dd>
            {lastPayment?.method && (
              <>
                <dt className="text-slate-600">{t("mobile.receipt_method")}</dt>
                <dd className="text-right capitalize">{String(lastPayment.method).replace(/_/g, " ")}</dd>
              </>
            )}
            {lastPayment?.reference && (
              <>
                <dt className="text-slate-600">{t("mobile.receipt_reference")}</dt>
        ...
        <footer className="mt-8 border-t border-slate-200 pt-4 text-center text-[10px] text-slate-400">
          {t("mobile.receipt_footer")}
          <br />
          {t("mobile.receipt_generated_on", { datetime: new Date().toLocaleString() })}
        </footer>
      </div>
    </div>
  );
